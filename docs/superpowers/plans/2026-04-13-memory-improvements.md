...
- [x] запустити повний `npm test` — всі тести зелені
- [x] typecheck усіх workspaces
- [x] ручна перевірка (опціонально): живий чат, сказати «запам'ятай що мене звати Іван» → `/запам'ятай` fact importance=10 у БД; наступне питання «як мене звати» → у відповіді використано fact, у UI плашка «Згадав: user.name=Іван»; `/забудь user.name` → наступне питання не знає імені
- [x] перевірити що існуючі 398+ тестів не зламалися

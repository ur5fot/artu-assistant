...
- [x] manual smoke (описание в Post-Completion): отправить в Discord "Изменить user.nickname" → "Name" → "Изменить" в течение 1.5с — LLM должен отработать один раз и либо выполнить, либо объяснить почему не может. (Manual-only step — агент не может выполнить в Discord. Сценарий задокументирован в Post-Completion; верифицировать в dev после деплоя.)
- [x] `npm run lint` — чисто. (В репо нет lint script — ни в root `package.json`, ни в одном из workspace пакетов. TypeScript `tsc --noEmit` выступает как статический анализатор, уже верифицирован в Task 3.)
- [x] полный `npm test` — 100% pass. (75 test files / 785 tests — все green.)

### Task 5: [Final] Обновить документацию

- [x] обновить `AGENTS.md` / `README.md` если там описан flow сообщений — добавить упоминание debounce и env `DISCORD_COALESCE_MS`. (README.md в репо отсутствует — обновлён только `AGENTS.md`: env block, Discord env-vars list, Architecture-секция с описанием burst coalescing и ссылкой на `memory/service.ts` `runIndexTurn`.)
- [x] обновить CLAUDE.md (project knowledge) если есть секция про message handling. (В репо нет CLAUDE.md — Glob `**/CLAUDE.md` пуст. Пункт неприменим.)

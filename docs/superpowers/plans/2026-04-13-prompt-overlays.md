...
- [x] перевірити що всі вимоги зі спеки реалізовані (get/set/clear, ліміт 10k, роздільники, помилки комбінацій)
- [x] `npm run typecheck` (якщо є в скриптах) або `tsc --noEmit` для `@r2/server` і нового пакету
- [x] `npm test` — повний test suite зелений
- [x] ручний тест локально (опціонально, але бажано): `OLLAMA_DEBUG=1 npm run dev`, у чаті `/клод-промпт відповідай коротко`, наступний запит до Claude → у логах видно що overlay у system message — покрито автотестами (chat.test.ts + prompts.test.ts + tool-prompt-overlay tests), ручний запуск залишено на юзера

### Task 6: [Final] Оновити документацію

- [x] додати коротку секцію у `AGENTS.md` про slash-команди `/клод-промпт`, `/лама-промпт` — usage, прапорці, ліміт
- [x] якщо спека `docs/superpowers/specs/2026-04-13-prompt-owerlays-design.md` містить щось що змінилося під час реалізації — додати примітку внизу «Реалізовано з відхиленнями: …»

...
- [x] в `imap-client.test.ts` расширить `makeClientStub`: опция `uidValidity?:
...
- [x] в `imap-client.ts` добавить `export async function getUidValidity(account:
...
- [x] прогнать `npm -w @r2/server test -- imap-client` — зелёно до Task 3.

### Task 3: Detect & self-heal `UIDVALIDITY` change в poller (TDD)

- [x] в `multi-account-poller.test.ts` написать падающие тесты (новые опции
  ...
  - **first-tick без изменений**: нет строки → `maxUidProbe` →
    ...
    тике через backfill.
- [x] в `multi-account-poller.ts` расширить типы/`TickParams`:
...
- [x] реализовать в ongoing-пути, **между** `const sinceUid = ...getLastSeenUid`
...
- [x] прогнать `npm -w @r2/server test -- multi-account-poller` — зелёно до Task 4.

### Task 4: Wiring в index.ts + integration-тест

- [x] в `index.ts` (стр. 54) добавить `getUidValidity` в импорт из
  `./emails/imap-client.js`.
- [x] в `startEmailPoller({...})` (605–638) добавить:
...
- [x] создать `packages/server/src/__tests__/email-poller.integration.test.ts`
...
- [x] прогнать `npm -w @r2/server test -- "email-poller|multi-account-poller"` —
  unit + integration зелёные.

### Task 5: Документация + приёмка

- [x] README (раздел про email watcher) + `AGENTS.md`: короткий блок про

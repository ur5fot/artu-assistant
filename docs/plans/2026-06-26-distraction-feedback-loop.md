...
- [x] implement domain detection (known-domain whole-word scan + `\b([a-z0-9-]+)\.(com|org|net|tv|io|me)\b` host extractor) then first-meaningful-word fallback (strip leading bracketed counters, emoji, `#`/`@`, punctuation; first token length ≥2 and not all-digits)

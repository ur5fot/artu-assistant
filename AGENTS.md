...
В общем Dedus Lab workspace этот репозиторий лежит в:

```text
/Users/dim/code/dedus-lab/R2-D2
```

Это зона разработки R2 / Artu. Материалы стримов, OBS, YouTube и сценарии живут
рядом в `../dedus_dev/`; не смешивать их с кодом R2 без явной причины.
...
| /fetch | web_fetch | Завантажити сторінку або кілька URL після пошуку |
...
| /погода | weather | Прогноз на 3 дні через Open-Meteo; без location використовує домашні координати |
...
- `/english [action:stop]` — English tutor (gated `ENGLISH_TUTOR_ENABLED`). No arg: starts/resumes placement if not done, continues an active lesson, or generates a new one. `action:stop` closes the active lesson/placement.
...
- `emailDigest` publish → the compact one-message digest now rides with a Discord **string select-menu** under the list (built by `emailDigest.helpers.ts:buildDigestMenu(rows, includedIds)` → plain `ComponentData` `{type:'select'}`, realised at the `bot.ts` boundary by `buildComponentsFromData`). One option per included row: `label = "{emoji} {sender} — {subject}"` and `description = snippet`, both clamped to Discord's 100-char limit; `value = String(row.id)`; capped at 25 options; customId `email_digest:pick`, placeholder `Выбери письмо для действия`. Picking an email opens an **ephemeral** action card (`interactions.ts:handleEmailDigestPick`, routed via the new `routeStringSelectMenu` branch — `routeInteraction` now handles `isStringSelectMenu()`) showing sender / subject / clean snippet plus one row of five buttons: `Разобрать` (`email_digest:dismiss:<id>`, success), `Ответить` (`email_draft:start:<id>`, **reuses** the urgent draft handler), `🙈 Отправитель` (`email_suppress:sender_start:<id>`, **reused**), `🙈 Тема` (`email_suppress:subject_start:<id>`, **reused**), `Полный текст` (`email_digest:fulltext:<id>`). So the digest and the urgent ping share one action surface — only `dismiss` and `fulltext` are new. A pick on a row that's already gone replies ephemerally `⚠️ Письмо больше недоступно`. **Разобрать** (`handleEmailDigestDismiss`) applies the same awaiting predicate as the `emails_dismiss` tool (`delivered_at IS NULL AND (urgent_pinged_at IS NULL OR urgent_pinged_at < 0)`) → `markDelivered([id], now)` + ephemeral `✓ Разобрано`; idempotent — a second click on an already-handled row honestly reports `Уже разобрано` (so it's gone from the next digest). **Полный текст** (`handleEmailDigestFullText`) `deferReply`s ephemerally, then `imapClient.fetchFullBody(account, message_uid)` (already tag-free thanks to the decode-chokepoint HTML→text pass) and `editReply`s the subject + clamped body; an IMAP/account error surfaces as an ephemeral notice rather than throwing.
...
- `englishLesson` (English tutor, iter 1) — proactive daily English lesson via DM, gated `ENGLISH_TUTOR_ENABLED=true`, self-gate pattern mirrors `morningBrief`. **Schema (`db.ts`, additive):** `tutor_profile` (single row: `level` CEFR, `placement_state` none|in_progress|done, `placement_payload` JSON, `daily_hour`, `paused`), `tutor_lesson` (JSON `payload`, `status` state machine, `current_ex`, `score`), `tutor_progress` (`topic` PK, `attempts`, `correct`, `mastery` EWMA, `last_at`). **Modules (`tutor/`):** `store.ts` (CRUD + mastery EWMA via `recordAttempt`), `placement.ts` (`startPlacement`/`assessPlacement` — Claude-generated 5–10 MCQ ascending-difficulty test → CEFR level; stepwise `beginPlacement`/`recordPlacementAnswer`/`finishPlacement` persist `placement_payload` so a mid-flight LLM failure is resumable — both the chat hook and `/english` retry `finishPlacement` when fully answered but not yet finalized, rather than silently no-oping or restarting), `lesson-generator.ts` (Claude-authored lesson JSON: explanation + MCQ/free exercises, retried once then throws), `grader.ts` (`gradeMcq` deterministic; `gradeFree` LLM verdict `correct|partial|wrong` + RU feedback, never fabricates on failure), `session.ts` (`awaiting_mcq → awaiting_free → done` state machine; `routingState()` reports whether a placement/free-answer is pending). **Discord surface (`channels/discord/tutor-handlers.ts`):** `/english` (see Slash commands above), `tutor:mcq:<lessonId>:<exIdx>:<choice>` buttons (button domain `tutor`), and `routeTutorMessage` (the free-text/placement-answer hook). **Routing precedence (`bot.ts`):** an incoming DM is checked against `routingState()` *before* it's saved to `chat_messages` or handed to the coalescer — an active placement/free-answer routes straight to the tutor and is graded individually (bypassing both the shared history and burst-coalescing, so tutor turns never pollute the assistant's context and rapid answers aren't collapsed); if that routing resolves to "not routed" (tutor state cleared while the turn was queued behind an earlier one), the message falls through to normal chat handling rather than being dropped. **Daily delivery:** `cognition/handlers/englishLesson.ts` (flag + placement done + local hour ≥ `ENGLISH_TUTOR_HOUR` + not quiet hours + not paused + no active lesson + not already published today) generates a lesson via the same generator and posts explanation + first exercise (MCQ buttons or a free-answer prompt). Env: `ENGLISH_TUTOR_ENABLED`, `ENGLISH_TUTOR_HOUR` (0–23, default 9), `ENGLISH_TUTOR_MODEL`.
- HTML→text at the decode chokepoint — `emails/imap-client.ts:decodePickedText(buf, picked)` is the single funnel for all three body paths (`fetchNewMessages` snippet, `fetchByMessageId` draft thread, `fetchFullBody` full body). After `decodeBodyPart(...)` it runs `emails/html-to-text.ts:htmlToText(decoded)` **only when** `picked.type === 'text/html'` (the server-declared part type), else returns the decoded text untouched. `htmlToText` drops `<script>`/`<style>` (with contents), turns block boundaries (`<br>`, `</p>`, `</div>`, `</li>`, `</tr>`, `</h1>`–`</h6>`) into newlines, strips remaining tags, decodes named + numeric HTML entities, and collapses blank-line/space runs. Because it's gated on the declared type, a plain-text body containing literal `<`/`&` is never mangled. One guard fixes the lot at once: the digest snippet, the importance **scorer** input (`scorer.ts` consumes `row.snippet`), `emails_get` body, the digest's `Полный текст` action, and the draft-thread context — so raw `<!DOCTYPE html>…` no longer leaks into previews or the LLM.
